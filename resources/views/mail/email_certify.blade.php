...
<div style="margin:30px auto;width:600px;border:10px solid #f7f7f7">
    <div style="border:1px solid #dedede">
        ...
        <p style="margin:20px 0 0;padding:30px 30px 50px;min-height:200px;height:auto !important;height:200px;border-bottom:1px solid #eee">
            @if($isEmailChange)
                <b>{{ $nick }}</b> 님의 E-mail 주소가 변경되었습니다.<br><br>
            @endif

            아래의 주소를 클릭하시면 인증이 완료됩니다.<br>
            <a href="{{ $url }}" target="_blank"><b>{{ $url }}</b></a><br><br>

            회원님의 성원에 보답하고자 더욱 더 열심히 하겠습니다.<br>
            감사합니다.
        </p>
        <a href="{{ route('login') }}" target="_blank" style="display:block;padding:30px 0;background:#484848;color:#fff;text-decoration:none;text-align:center">{{ Cache::get('config.homepage')->title }} 로그인</a>
    </div>
</div>
