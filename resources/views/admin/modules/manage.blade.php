...
@section('title')모듈 관리 권한 설정 | {{ Cache::get('config.homepage')->title }}@endsection
...
<div class="body-head">
    <div class="pull-left">
        ...
        <ul class="fl">
            ...
            <li class="depth">모듈 관리 권한 설정</li>
        </ul>
    </div>
</div>
<div id="body_tab_type2">
    <span class="txt">설치된 모듈의 관리권한을 설정합니다.</span>
</div>
<div class="body-contents">
    ...
    <div id="auth_list">
        <ul id="adm_btn">
            <li>
                <button type="button" class="btn btn-sir" onclick="location.href='{{ route('admin.modules.manage') }}'">
                     전체보기
                </button>
            </li>
            <li>
            ...
            <li>
                <span>
                    설정된 모듈 관리 권한 {{ $manageAuthList->total() }}건
                </span>
            </li>
        </ul>

        <div id="adm_sch">
            <form class="form-horizontal" role="form" method="GET" action="{{ route('admin.modules.manage') }}">
            ...
            <table class="table table-striped box">
                <thead>
                    <tr>
                        ...
                        <th>
                            <a class="mb_tooltip" href="{{ route('admin.modules.manage')."?keyword=$keyword&order=email&direction=" }}{{ $order == "email" ? $direction : "asc" }}">회원이메일</a>
                        </th>
                        <th>
                            <a class="mb_tooltip" href="{{ route('admin.modules.manage')."?keyword=$keyword&order=nick&direction=" }}{{ $order == "nick" ? $direction : "asc" }}">닉네임</a>
                        </th>
                        <th>메뉴</th>
                        <th>권한</th>
                    </tr>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="td_email"><a href="{{ route('admin.modules.manage') }}?keyword={{ $auth->user_email }}">{{ $auth->user_email }}</a></td>
                        <td class="td_nick">
    ...
    <div id="adm_alert">
        <span class="adm_alert_txt">다음 양식에서 회원에게 모듈의 관리 권한을 부여하실 수 있습니다.<br>
    ...
    <div id="authlist_add" class="panel panel-default">
        ...
        <div class="panel-body row">
            <form class="form-horizontal" role="form" method="POST" action="{{ route('admin.manageAuth.store') }}">
                ...
                <div class="form-group">
                    <label for="" class="col-sm-2 control-label">접근 가능 모듈</label>
                    <div class="col-sm-3">
                        <select class="form-control required" name="menu" placeholder="접근 가능 모듈">
